...
class InvoiceGeneralController extends Controller
{
    ...
    public function actionIndex(){
        ...
        $number = InvoiceGeneral::find()->where(['YEAR(date)' => date('Y'), 'MONTH(date)' => date('m')])->count();
        $invoiceId = ($number + 1) . "/" . (( date('Y') + 543 ) - 2500);
        
        $number = RecieptGeneral::find()->where(['YEAR(date)' => date('Y'), 'MONTH(date)' => date('m')])->count();
    ...
    public function actionCreate(){
        ...
        $number = InvoiceGeneral::find()->where(['YEAR(date)' => date('Y'), 'MONTH(date)' => date('m')])->count() + 1;
